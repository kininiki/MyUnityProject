...
namespace Fungus
{
    ...
    public class Flowchart : MonoBehaviour, ISubstitutionHandler
    {
        ...
        protected virtual void OnEnable()
        {
            ...
            {
                // Delay cleanup twice so Unity has time to fully deserialize
                // all components. This prevents commands from being removed
                // when entering edit mode or after reloading scripts.
            ...
            {
                // Running in play mode within the editor
                StartCoroutine(DelayedRuntimeCleanup());
            }
#else
        ...
        protected virtual IEnumerator DelayedRuntimeCleanup()
        {
            // Wait two frames to ensure all commands are deserialized and
            // connected to their blocks before cleanup runs.
            yield return null;
            yield return null;
            if (this != null)
            {
