...
public class RewardedElixir : MonoBehaviour, IUnityAdsLoadListener, IUnityAdsShowListener
{
    ...
    private const int _maxAdViews = 10; // Максимальное количество просмотров рекламы
    private int _currentAdViews = 0; // Текущее количество просмотров рекламы
    private int _adViewsForOneRuby = 5; // Количество просмотров рекламы для начисления 1 эликсира
    ...
    private void HandleAdFailure()
    {
        ...
        // Начисляем рубины за каждые 5 просмотров
        if (_currentAdViews % _adViewsForOneRuby == 0)
        {
            GameCloud.Instance.UpdateResource("PLAYER_ELIXIR", 1);
        }
    ...
    public void OnUnityAdsShowComplete(string adUnitId, UnityAdsShowCompletionState showCompletionState)
    {
        if (adUnitId.Equals(_adUnitId) && showCompletionState.Equals(UnityAdsShowCompletionState.COMPLETED))
        {
            ...
            // Начисляем эликсиры за каждые 5 просмотров
            if (_currentAdViews % _adViewsForOneRuby == 0)
            {
                GameCloud.Instance.UpdateResource("PLAYER_ELIXIR", 1);
            }
