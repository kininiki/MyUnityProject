...
public class RewardedAdsButton : MonoBehaviour, IUnityAdsLoadListener, IUnityAdsShowListener
{
    ...
    private const float _timerDuration = 86400f; // Таймер 4 часа (в секундах)
    private const int _maxAdViews = 5; // Максимальное количество просмотров рекламы
    private int _currentAdViews = 0; // Текущее количество просмотров рекламы
    private int _adViewsForOneRuby = 1; // Количество просмотров рекламы для начисления 1 рубина
    ...
    private void HandleAdFailure()
    {
        ...
        // Начисляем рубины за каждые 2 просмотра
        if (_currentAdViews % _adViewsForOneRuby == 0)
        {
            GameCloud.Instance.UpdateResource("PLAYER_RUBY", 1);
        }
    ...
    public void OnUnityAdsShowComplete(string adUnitId, UnityAdsShowCompletionState showCompletionState)
    {
        if (adUnitId.Equals(_adUnitId) && showCompletionState.Equals(UnityAdsShowCompletionState.COMPLETED))
        {
            ...
            // Если счетчик рекламы достигает 7/7, запускаем таймер
            if (_currentAdViews >= _maxAdViews)
            {
                _timeRemaining = _timerDuration;
                _isTimerRunning = true;
